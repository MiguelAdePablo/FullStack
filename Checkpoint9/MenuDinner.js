 ...
 * VERSIÓN: Accent-insensitive matching + Unicode normalization
 */
...
// DECISIÓN: Normalización Unicode para comparación accent-insensitive
// Explicación: NFD descompone "é" → "e" + "◌́", luego eliminamos los diacríticos [\u0300-\u036f]
function normalizeForComparison(str) {
    if (typeof str !== 'string') return '';
    return str
        .normalize('NFD')                          // Descomposición canónica
...
function selectItemFromCategory(categorynombre, items) {
    ...
    while (!isValidSelection) {
        ...
        // DECISIÓN: Normalización para comparación accent/case-insensitive
        const normalizedInputForSearch = normalizeForComparison(normalizedInput);
        
        // DECISIÓN: Búsqueda lineal con normalización en ambos lados de la comparación
        const foundItem = items.find(item => 
            normalizeForComparison(item.nombre) === normalizedInputForSearch
        );
        ...
        if (foundItem) {
            ...
            errorMsg += "Puedes seleccionar por:\n  - NÚMERO: 1, 2, 3...\n  - NOMBRE (con/sin acentos, mayús/minús)\n\n";
...
function startBottegaDiner() {
    console.log("Iniciando Bottega Diner System - Versión Híbrida Accent-Insensitive...");
...
// ============================================================================
// BLOQUE DE PRUEBAS Y DEMOSTRACIÓN
// ============================================================================

if (typeof window !== 'undefined') {
    // DECISIÓN: Exponer función de normalización para pruebas en consola del navegador
    window.normalizeForComparison = normalizeForComparison;
    
    // DECISIÓN: Función de utilidad para pruebas rápidas sin interfaz gráfica
    window.testAccentMatching = function(testCases = []) {
        console.group("🧪 Pruebas de normalización de acentos");
        
        const defaultTests = [
            { input: "Café Cortado", expected: "cafe cortado" },
            { input: "Cafe Cortado", expected: "cafe cortado" },
            { input: "CAFÉ CORTADO", expected: "cafe cortado" },
            { input: "té chai", expected: "te chai" },
            { input: "Te Chai", expected: "te chai" },
            { input: "Tiramisú", expected: "tiramisu" },
            { input: "Tiramisu", expected: "tiramisu" },
            { input: "  Ensalada César  ", expected: "ensalada cesar" }
        ];
        
        const testsToRun = testCases.length > 0 ? testCases : defaultTests;
        
        testsToRun.forEach((test, idx) => {
            const result = normalizeForComparison(test.input);
            const passed = result === test.expected;
            console.log(`Test ${idx + 1}: ${passed ? '✅' : '❌'} "${test.input}" → "${result}" ${passed ? '' : `(esperado: "${test.expected}")`}`);
        });
        
        console.groupEnd();
    };
    
    // DECISIÓN: Función para simular búsqueda de item sin interfaz (para debugging)
    window.findItemByName = function(itemName, categoryKey = null) {
        const normalizedSearch = normalizeForComparison(itemName);
        
        // Búsqueda en todo el menú si no se especifica categoría
        for (const menuType of Object.keys(RESTAURANTE_DATO)) {
            if (categoryKey && menuType !== categoryKey) continue;
            
            const menu = RESTAURANTE_DATO[menuType];
            for (const categoryName of Object.keys(menu)) {
                const found = menu[categoryName].find(item => 
                    normalizeForComparison(item.nombre) === normalizedSearch
                );
                if (found) return { found, menuType, categoryName };
            }
        }
        return null;
    };
